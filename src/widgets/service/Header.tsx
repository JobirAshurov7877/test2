...
const SubServicesHeader: React.FC<CategoryProps> = ({ service }) => {
  ...
  const curretLanguage = useLocale();
  const itemId = "1";
  const dispatch = useDispatch();
  useEffect(() => {
    if (itemId) {
      (dispatch as ThunkDispatch<RootState, unknown, UnknownAction>)(
        fetchServiceRootData({ rootId: itemId as string, lang: curretLanguage })
      );
    }
  }, [dispatch, curretLanguage, itemId]);

  const handleBook = () => {
    navigate.push(
      `/${curretLanguage}/services/booking-process/service/${service.id}/${service.slug}`
    );
  };
  ...
  return (
    <Container>
      <Box>
        <Image src={imagesAPI + service.image} alt="" />
        <Desc>
          <h1>{service.title}</h1>
          <p>
            {service.description === "."
